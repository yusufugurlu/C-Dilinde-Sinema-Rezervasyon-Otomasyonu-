...
struct Bilet{
	...
	int SalonId;
	int SeansId;
	//Tam fiyat mý öðrenci fiyatý  mý
	int FiyatTuru;
...
else
{
      while(!feof(fp))
      {
		fscanf(fp,"%d %d %d %d %d %d %d\n",&BBilet[BiletSayisi].Id,&BBilet[BiletSayisi].KullaniciId,&BBilet[BiletSayisi].FilmId,&BBilet[BiletSayisi].KoltukId,&BBilet[BiletSayisi].SalonId,&BBilet[BiletSayisi].SeansId,&BBilet[BiletSayisi].FiyatTuru);
		BiletSayisi++;
...
int IdGoreSeansGetir2(int Id,int SalonID){
	int deger=-1;
	if(Id==999)
	{
		sleep(3);
...
for(int i=0;i<SalonSeansSayisi;i++)	
{
  ...
  if(SSalonSeans[i].SeansId==Id && SSalonSeans[i].SalonId==SalonID)	
	{
...
printf("KOLTUK SECIMINE HOSGELDINIZ \n");
TarihGoster();
printf("\nSayin:%s %s",KKullanici[kullaniciId].Adi,KKullanici[kullaniciId].Soyad);
printf("\nSalon Adi:%s\n",SSalon[SalonId].Adi);
printf("\nFilmin Adi : %s",FFilmler[FilmId].Adi);
IdGoreTurGetir(FFilmler[FilmId].FTuru);
printf("Filmin yonetmeni: %s\n",FFilmler[FilmId].FYonetmeni);
...
//Burda rezervayon için gerken son aþama, koltuk seçildiði ve ödeme yapýldýgý metod
void OdemeYeri(int SeansId,int FilmId,int SalonId,int KullaniciId)
{
if(KullaniciId==-1)
{
...
else if(RezerveKoltuk==999)
{
	printf("Ana menuye gidiliyor.");
	sleep(3);
	AnaIslem();
}
...
while(i<RezerveKoltuk)
{
	...
	if(a==-1)
	{
			...
			printf("%s\n",RezerveKoltukDizi[i]);
			GeriDon:
			printf("\n1-Ogrenci fiyatý=8TL\n2-Tam fiyati=14TL\nSecim yapiniz: ");
			scanf("%d",&TamOgrenci[i]);
			if(TamOgrenci[i]!=1 && TamOgrenci[i]!=2 )
			{
...
for(int i=0;i<RezerveKoltuk;i++)
{
	...
	TOgrenci=0;
	fprintf(fp,"%d %d %d %d %d %d %d\n",BiletSayisi,KullaniciId+1,FilmId+1,koltukId,SalonId+1,SeansId,TOgrenci);
BiletSayisi++;
}
...
//Kullanici Girisi bu metodta yapýldý.
...
//Odeme yerine gelmeden önce burda kullanici login olumuþ mu kontrolü yapýlýr.
...
//Salonu seçtikten sonra seansSecime gelinir.
void SeansSecimi(int Id,int FilmId){
int secim;
char AdSoyad[100]="";	
...
printf("Filmin suresi: %d.dk",FFilmler[FilmId-1].Saati);
printf("\n--------------------------------------");
for(int i=0;i<SalonSeansSayisi;i++)
{
...
int Kontrol=IdGoreSeansGetir2(secim,Id);
switch(Kontrol)
{
...
//Burda vizyondaki filmler ve hangi  salonda gösterilir.
void Rezervasyon()
{
...
for(int i=0;i<BiletSayisi;i++)
	{
		if(DurumId==BBilet[i].KullaniciId)
		{
			...
			if(BBilet[i].FiyatTuru==1)
			printf("Fiyat: 8 TL(ogrenci)\n");
			else
			printf("Fiyat: 14 TL(Tam)\n");
	...
	int BID;
	int a;
	printf("\nAna menuye donmek icin 999 basin \n");
	printf("Silmek istediginiz (ID) numarasini giriniz: ");
	scanf("%d",&BID);
	if(BID==999)
	AnaIslem();
	int iSayac=-1;
	...
	{
		char sKontrol[3];
		printf("Silmek icin (y) iptal icin(n) basiniz.");
		scanf("%s",sKontrol);
	if(strcmp(sKontrol,"y")==0 || strcmp(sKontrol,"Y")==0  )
	{
...
char cDosyaAdi5[100] ="Bilet.txt";
remove(cDosyaAdi5);
 FILE *f1,*fp2;
fp2=fopen(cDosyaAdi5,"a");
fclose(fp2);
f1=fopen(cDosyaAdi5,"a");
...
remove(cDosyaAdi2);
 FILE *f5,*f6;
f5=fopen(cDosyaAdi2,"a");
fclose(f5);
f6=fopen(cDosyaAdi2,"a");
for(int t=0;t<KoltukSayisi;t++)
{
	if(KKoltuk[t].Id==KKoltuk[j].Id)
	{
		fprintf(f6,"%d %d %s %d\n",KKoltuk[t].Id,KKoltuk[t].SalonID,KKoltuk[t].KoltukNo,0);
	}
	else
	{
		fprintf(f6,"%d %d %s %d\n",KKoltuk[t].Id,KKoltuk[t].SalonID,KKoltuk[t].KoltukNo,KKoltuk[t].Durum);
	}
}
	fclose(f6);	
	}
			...
			{
					fprintf(f1,"%d %d %d %d %d %d %d\n",BBilet[i].Id,BBilet[i].KullaniciId,BBilet[i].FilmId,BBilet[i].KoltukId,BBilet[i].SalonId,BBilet[i].SeansId,BBilet[i].FiyatTuru);
			}
		...
		BiletSayisi=0;
		BiletleriCagir();
		printf("Isleminiz basarili sekilde gerceklesti...");
...
//Ipucuna göre bloke kaldýrma iþlemi yapýlýr.
void BlokeGuncelleme(int Durum,int Id)
{
...
tekrar:
printf("\n1-Rezervasyon Yap \n2-Rezervasyonu iptal et\n3-Filmlerle ilgili yorumlar \n4-Kullanici Islemleri(Oturum) \n5-Cikis");	
if(DurumId==-1)
printf("\nLutfen oturum acmayi unutmayiniz... Iyi eglenceler");
printf("\nYapmak istediginiz islemi seciniz: ");
scanf("%d",&Secim);	
switch(Secim){
